...
<body>
	    ...
	    <div class="modal-dialog" role="document">
	        <div class="modal-content">
	            ...
	            <div class="modal-footer">
	                ...
	                <button type="button" class="btn btn-default" data-dismiss="modal" id="modal-addcart">장바구니 담기</button>	
	...
	<script>
	    ...
	    $(document).ready(function() {
	        ...
	        // 모달이 열릴 때 Ajax 요청을 보내서 DB 조회 결과를 가져옴
	        $('#myModal').on('shown.bs.modal', function() {
	            ...
	            $.ajax({
	                ...
	                success: function(response) {
	                    ...
	                    //영양소 테이블 출력
	                    //DB 조회 결과를 테이블에 동적으로 추가
	                    for(var i = 0; i < 7; i++) {	// for (var i = 0; i < response.length; i++) {
	                        var row = $('<tr>');
	                        if(i == 0) {
	                            row.append($('<td>').text('칼로리'));	//row.append($('<td>').text(response[i].column1));
	                            row.append($('<td>').text(response.kcal));	//row.append($('<td>').text(response[i].prodNum));
	                        } else if(i == 1) {
